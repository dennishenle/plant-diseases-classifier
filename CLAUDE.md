...
4. Main loop — two-phase training: head-only for `--freeze-epochs` epochs (backbone frozen), then full fine-tune at `lr × 0.1`. Uses AdamW + CosineAnnealingLR + CrossEntropyLoss with label smoothing 0.1.
...
**`plot_history.py`** — reads `output/history.csv`, prints a per-epoch table, saves `output/history.png` with loss and accuracy curves. Optionally marks the freeze/unfreeze boundary with `--freeze-epochs`.
